...
import { AppwriteConfig, APPWRITE_CONFIG, APPWRITE_CONFIG_DEFAULT_VALUE } from './appwrite.config';
...
export class NgxAppwriteModule {
  static forRoot(
    config: AppwriteConfig = APPWRITE_CONFIG_DEFAULT_VALUE,
    @Optional() @SkipSelf() parentModule?: NgxAppwriteModule
  ): ModuleWithProviders<NgxAppwriteModule> {
    if (parentModule) {
      throw new Error('NgxAppwriteModule is already loaded. Import it in the AppRootModule only');
    }
